...
Генерація світу вже винесена в чисті модулі, тому такі перевірки мають лишатися unit-style і не залежати від DOM.
...
- [ ] нові runtime-файли, CSS і JSON або додані в `STATIC_ASSETS`, або явно відкладені до фінального PWA-проходу.
...
### Зміни у винагородах або звуках

Перевірити:

- правильна відповідь викликає `onSolved()` один раз;
- зірка й конфетті не блокують фокус і кліки;
- `prefers-reduced-motion` не залишає критичну дію невидимою;
- звук запускається тільки після дії користувача;
- тести проходять без доступного Web Audio API.

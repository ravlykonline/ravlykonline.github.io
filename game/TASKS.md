...
## 29. Формат та fallback задачі `magic-square`

Тип `magic-square` показує сітку 3×3 де одна клітинка порожня (`null`). Дитина обирає правильний символ з трьох варіантів.

**Правило сітки:** кожен рядок і кожен стовпець містять рівно по одному екземпляру кожного з трьох символів (латинський квадрат 3×3).

Формат варіанту у JSON-категорії або `magic-square-variants.js`:
...
```js
{
    ...
    grid: ['🌼', '🍄', '🍃',
           '🍄', '🍃', '🌼',
           '🍃', null, '🍄'],   // рівно 9 клітинок, рівно одна null
    choices: [
        { id: 'flower',   label: '🌼' },
        { id: 'mushroom', label: '🍄' },
        { id: 'leaf',     label: '🍃' }
...
Ключові правила:
- `grid` — рівно **9 клітинок**, рівно **одна `null`**.
- `choices` — рівно **3 варіанти** `{id, label}`.
- У сітці та серед choices використовуються **рівно 3 унікальні символи**.
- `MagicSquareTask.createTask()` використовує `entry.variant ?? pickVariant(random)` — якщо JSON-entry не передано, обирається випадковий варіант із `magic-square-variants.js`.
- JS fallback перевіряється тестом `test-game.js` на коректність структури (9 клітинок, одна null, 3 choices, correctChoiceId серед choices, рівно 3 унікальних символи).

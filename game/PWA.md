...
- `skipWaiting()` викликається всередині `.then()` після `cache.addAll()` — race condition виправлено;
...
1. Додати або оновити import у `js/tasks/task-catalog.js`.
2. Перевірити, що задача доступна через `TaskCatalog`.
3. Додати файл у `STATIC_ASSETS` у `sw.js`, якщо категорія має працювати offline.
4. Оновити `STATIC_CACHE` version.
...
## 10. Що робити при перейменуванні файлу
...
Банер оновлення Service Worker **реалізовано** в `js/pwa/register-sw.js`.

При виявленні нової версії SW (через `updatefound` / `statechange`) у верхній частині екрану з'являється ненав'язливий банер «Оновити». Дитина або вчитель можуть клікнути для оновлення або закрити банер. Стилі банера визначені в `style.css` (`#sw-update-banner`).
...
## 16. Рекомендований майбутній рефакторинг PWA

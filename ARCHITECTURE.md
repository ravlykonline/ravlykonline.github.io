...
- `js/modules/canvasStateController.js` — текстовий навчальний стан полотна й журнал максимум 50 завершених примітивів; журнал не зберігається між сесіями. Стан живе в модальному вікні з тулбара, щоб не забирати місце в полотна й не змінювати вже намальоване.
...
### 7.7. Свіжість неверсійних ES-модулів

Release token версіонує лише точки входу (`main.js?v=TOKEN`). Граф `js/modules/*.js` імпортується голими URL, які між релізами не змінюються, тому свіжу точку входу можна поєднати зі старим модулем і дістати `does not provide an export named ...`, що валить увесь граф і лишає сторінку без обробників.

Гарантія свіжості — precache Service Worker через `new Request(url, { cache: 'reload' })`: він обходить HTTP-кеш браузера, тому в кеш потрапляють байти поточного релізу.

Заголовками це не вирішується: вимірювання 2026-09-08 показало, що Cloudflare Pages не застосовує `Cache-Control` із `_headers` для цих ресурсів, а підставляє власний `max-age`. Не додавати туди правил кешування: вони виглядають як захист, але ним не є.

### 7.8. Shared HTML partials

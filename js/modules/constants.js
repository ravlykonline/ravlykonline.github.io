...
export const ERROR_MESSAGES = {
    ...
    UNEXPECTED_EXECUTION_ERROR: 'Не вдалося виконати програму. Спробуй запустити її ще раз.',
    GIF_GAME_UNSUPPORTED: 'Запис GIF поки доступний для малювання. Для гри збережи код або малюнок.',
    GIF_CAPTURE_LIMIT_SAVED: 'Збережено початок анімації: досягнуто ліміту запису.',
    GIF_CREATE_ERROR: 'Не вдалося створити GIF. Спробуй ще раз.',
    ...
    UNCLOSED_STRING: 'У тексті не закриті лапки. Додай " наприкінці тексту.',
    UNKNOWN_COMMAND: (command) => 'Я не знаю команди "' + command + '". Перевір написання за списком команд. Якщо це твоя функція — спочатку створи її.',
    UNKNOWN_COLOR: (colorName) => 'Я не знаю кольору "' + colorName + '". Подивись назви кольорів у палітрі.',
    REPEAT_EXPECT_NUMBER: 'Після "повторити" треба вказати, скільки разів повторювати.',
    INVALID_REPEAT_COUNT: (value) => 'Кількість повторень "' + value + '" не підходить. Тут має бути ціле число.',
    ...
    WHILE_EXPECT_OPEN_PAREN: 'Після умови в команді "поки" потрібна дужка "(". Наприклад: поки x < 10 ( ... ).',
    BREAK_OUTSIDE_LOOP: 'Команда "стоп" працює тільки всередині циклу "повторити" або "поки".',
    ...
    CONDITION_VALUE_INVALID: 'В умові не вдалося отримати число. Перевір обчислення: ділити на нуль не можна.',
    ...
    AST_TOO_LARGE: 'Програма надто велика (більше ' + MAX_AST_NODES + ' частин). Спробуй розбити її на менші кроки або спростити повторення.',
    CANVAS_OUT_OF_BOUNDS: 'Равлик дійшов до краю полотна. Зменш відстань або поверни його.',
    SAVE_IMAGE_ERROR: 'Не вдалося зберегти малюнок. Спробуй ще раз.',
    SAVE_IMAGE_SECURITY_ERROR: 'Браузер не дозволив зберегти малюнок (обмеження безпеки). Спробуй зробити скріншот.',
    SAVE_CODE_ERROR: 'Не вдалося зберегти код. Спробуй ще раз.',
    SHARE_LINK_INVALID: 'Посилання з кодом пошкоджене або неповне.',
    SHARE_LINK_TOO_LONG: 'Посилання з кодом завелике. Скористайся кнопкою "Код".',
    ...
    FUNCTION_ALREADY_EXISTS: (name) => 'Функція "' + name + '" вже існує. Обери іншу назву або видали попереднє оголошення.',
    FUNCTION_CALL_SYNTAX: (name) => 'Виклик "' + name + '" записано неправильно. Перевір дужки та аргументи: ' + name + '() або ' + name + '(значення).',
    ...
    WAIT_IN_GAME_MODE: 'Команда "чекати" не працює всередині "грати (...)". У грі рух і так оновлюється сам, багато разів за секунду.',

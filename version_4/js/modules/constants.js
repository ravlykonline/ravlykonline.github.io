...
export const ERROR_MESSAGES = {
    EXECUTION_IN_PROGRESS: 'Зачекай, попередній запуск ще виконується.',
    ...
    FUNCTION_NAME_RESERVED: (name) => `Назва "${name}" вже зайнята, придумай іншу.`,
    FUNCTION_NAME_INVALID: (name) => `Некоректна назва функції: "${name}". Використовуй літери/цифри/_.`,
    FUNCTION_NAME_CONFLICT_VARIABLE: (name) => `Назва "${name}" вже використовується як змінна.`,
    FUNCTION_PARAM_INVALID: (name) => `Некоректна назва параметра: "${name}".`,
    FUNCTION_PARAM_RESERVED: (name) => `Параметр "${name}" не може мати назву вбудованої команди.`,
    FUNCTION_DECLARATION_SYNTAX: 'Помилка в "створити": очікується формат створити назва(параметр) ( ... ).',
    FUNCTION_BODY_EMPTY: (name) => `Тіло функції "${name}" порожнє.`,
    FUNCTION_ALREADY_EXISTS: (name) => `Функція "${name}" вже існує.`,
    FUNCTION_CALL_SYNTAX: (name) => `Помилка виклику "${name}": очікується формат ${name}(число).`,
    FUNCTION_ARGUMENT_INVALID: (name, value) => `Некоректний аргумент "${value}" для виклику "${name}".`,
    VARIABLE_DECLARATION_SYNTAX: 'Помилка оголошення змінної: очікується формат створити назва = число.',
    VARIABLE_NAME_INVALID: (name) => `Некоректна назва змінної: "${name}". Використовуй літери/цифри/_.`,
    VARIABLE_NAME_RESERVED: (name) => `Назва "${name}" вже зайнята командою, придумай іншу.`,
    VARIABLE_NAME_CONFLICT_FUNCTION: (name) => `Назва "${name}" вже використовується як функція.`,
    VARIABLE_VALUE_INVALID: (name, value) => `Некоректне значення "${value}" для змінної "${name}".`,
    UNDEFINED_VARIABLE: (name) => `Змінну "${name}" не знайдено. Спочатку створи її: створити ${name} = число.`,
};

...
test('game exposes 20 levels and unlocks all tiles after level 10', () => {
  ...
  assert.equal(app.getLevelById(13)?.type, 'debug');
  assert.equal(app.getLevelById(16)?.type, 'debug');
  assert.equal(app.getLevelById(19)?.type, 'debug');
...
test('centered layouts keep start and apple away from the edges', () => {
  const { app } = bootstrapCore();

  for (const id of [1, 4, 11]) {
    const level = app.getLevelById(id);
    assert.ok(level.start.r > 0 && level.start.r < level.rows - 1, `Level ${id} start row should be centered`);
    assert.ok(level.apple.r > 0 && level.apple.r < level.rows - 1, `Level ${id} apple row should be centered`);
    assert.ok(level.start.c > 0 && level.start.c < level.cols - 1, `Level ${id} start col should be centered`);
    assert.ok(level.apple.c > 0 && level.apple.c < level.cols - 1, `Level ${id} apple col should be centered`);
  }
});
...
test('executed levels keep readable Ukrainian names and hints', () => {
  ...
  assert.equal(app.getLevelById(2)?.name, '\u0422\u0440\u0438 \u043a\u0440\u043e\u043a\u0438 \u0432\u0433\u043e\u0440\u0443');
